		...
		<script>
			$(document).ready(function(e) {				
					...
					$.ajax({
						...
						success: function(json) {
							if (json.flag === "true") {
								...
								$('#fileupload,#fileupload1,#fileupload2').fileupload({
				...
				$("#workSubmit").on("click",function(){
					...
					if($("#workFiles>.template-download>td>.name").length!=1 && (!$("#workstyle_A3").prop('checked') && $('#hqxuForm :input[name=workstyle]:checked').length==1) ){
						alert("请上传程序！");
						return false
					}
					if(!$("#workFiles>.template-download>td>.name").eq(0).data("url")&& (!$("#workstyle_A3").prop('checked') && $('#hqxuForm :input[name=workstyle]:checked').length==1) ){
						alert("请正确上传程序！");
						return false
					}
					var miniSiteUrl = "";
					...
					console.log(TEAMID,TEAMNAME_OLD,workname,workstyle,workUsingData,workSummary,workDetail,workFiles,workFilesUrl,miniSiteUrl,workPPT,workPPTUrl,workVideo,workVideoUrl);
					...
					$("#List #tabel-workFiles").html(workFiles);
					if(miniSiteUrl){
