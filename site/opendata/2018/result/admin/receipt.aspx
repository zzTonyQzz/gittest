...
<html lang="zh-cn">    
    ...
    <body>
          ...
          <div class="projects-header page-header">
        		<h2 class="text-center">作品提交表 - 开放数据竞赛2017<br/><small>回执单</small></h2> 
        		    
        		<p class="text-center"><a href="../api/resultReceiptGet.aspx?token=<%= hqxuNo %>" class="btn btn-primary">下载回执单</a></p>  		
      		</div>   
								...
								$.ajax({
										...
										success: function (json) {
												if (json.flag === "true") {
														...
														var workFilesUrl = "http://pcrc.library.sh.cn/FileUpload/hqxu/"+json.data[0].workFilesUrl.replace(/&nbsp;/g,"%20");
														if (workFiles)
															html += '<tr><td>程序</td><td><a href="'+workFilesUrl+'" target="_blank">'+workFiles+'<a></td></tr>';
														else
															html += '<tr><td>程序</td><td>未上传</td></tr>';
														var workLogo = json.data[0].workLogo;
														var workLogoUrl = "http://pcrc.library.sh.cn/FileUpload/hqxu/"+json.data[0].workLogoUrl.replace(/&nbsp;/g,"%20");
														if(workLogo)
															html += '<tr><td>LOGO</td><td><a href="'+workLogoUrl+'" target="_blank">'+workLogo+'<a></td></tr>';
														else
															html += '<tr><td>LOGO</td><td>未上传</td></tr>';													
														var miniSiteUrl = json.data[0].miniSiteUrl;
														if(miniSiteUrl)
															html += '<tr><td>微站访问地址</td><td>'+miniSiteUrl+'</td></tr>';
														var workPPT = json.data[0].workPPT;
														var workPPTUrl = json.data[0].workPPTUrl.split("#hqxu#");
														var tempS = "";
														$.each(workPPTUrl,function(index,element){
															var fileUrl =  decodeURIComponent(element.replace(/&nbsp;/g,"%20")).replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&nbsp;/g," ").replace(/&quot;/g,"\"").replace(/&#39;/g,"\'").replace(/\\\\/g,"\\").replace(/\\n/g,"\n").replace(/\\r/g,"\r");
															tempS += '<a href="http://pcrc.library.sh.cn/FileUpload/hqxu/'+fileUrl+'" target="_blank">'+element.split('_hqxu_')[1]+'<a/>&nbsp;&nbsp;'
														});
														html += '<tr><td>作品介绍PPT与PDF</td><td>'+tempS+'</td></tr>';
														var workVideo = json.data[0].workVideo;
														var workVideoUrl = "http://pcrc.library.sh.cn/FileUpload/hqxu/"+json.data[0].workVideoUrl.replace(/&nbsp;/g,"%20");
														if(workVideo)
															html += '<tr><td>作品介绍视频</td><td><a href="'+workVideoUrl+'" target="_blank">'+workVideo+'<a/></td></tr>';
